...
def serve_media_file(request, file_path):
    ...
    Maneja tanto archivos locales como de S3.
    """
    try:
        # Si el path empieza con 's3://', es un archivo de S3
        if file_path.startswith("s3://"):
            s3_key = file_path[5:]  # Remover 's3://' prefix
            return serve_s3_media_file(request, s3_key)
        ...
        if range_header:
            ...
            return handle_range_request(
                full_path, range_header, content_type, file_size
            )
        else:
...
def serve_s3_media_file(request, s3_key):
    ...
    Redirige a una URL prefirmada de S3.
    """
    try:
