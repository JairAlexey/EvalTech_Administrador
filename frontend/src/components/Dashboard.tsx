...
export default function Dashboard({ onNavigate, onLogout }: DashboardProps) {
  ...
  // Datos simulados para candidatos recientes
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
              ...
              <div className="flex items-center mt-4">
                <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium ${
                  summaryData.events.increasing ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                }`}>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h3 className="text-3xl font-bold text-gray-900 mt-2">{summaryData.candidates.total}</h3>
                </div>
              ...
              <div className="flex items-center mt-4">
                <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium ${
                  summaryData.candidates.increasing ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                }`}>
                  {summaryData.candidates.increasing ? 
                    <ArrowUp className="w-3 h-3 mr-1" /> : 
                    <ArrowDown className="w-3 h-3 mr-1" />
                  }
                  {summaryData.candidates.change}%
                </span>
                <span className="text-xs text-gray-500 ml-2">vs periodo anterior</span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
              ...
              <div className="flex items-center mt-4">
                <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium ${
                  summaryData.evaluations.increasing ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                }`}>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
              ...
              <div className="flex items-center mt-4">
                <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium ${
                  summaryData.completion.increasing ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                }`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-gray-900">Candidatos Recientes</h3>
                <button className="text-sm text-blue-600 font-medium hover:text-blue-800">
                  Ver todos
                </button>
              </div>
              ...
              <div className="space-y-4">
                {recentCandidates.map((candidate) => (
                  <div key={candidate.id} className="flex items-center p-3 hover:bg-gray-50 rounded-lg transition">
                    <div className={`w-10 h-10 rounded-full ${candidate.color} flex items-center justify-center text-gray-700 font-medium text-sm mr-4`}>
                      {candidate.initials}
                    </div>
                    <div className="flex-1">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs font-medium">{candidate.date}</p>
                    </div>
                  </div>
                ))}
              </div>
